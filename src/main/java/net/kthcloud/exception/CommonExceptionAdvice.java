package net.kthcloud.exception;
...
@ControllerAdvice  // 컨트롤러용 지원코드 -> 필수 : servlet-context.xml에 scan 코드를 넣음.
@Log4j2			   // 로그 출력용
public class CommonExceptionAdvice {
	// AOP (관심사의 분리) -> 예외처리용 코드를 분리한다.
	// 컨트롤러에서 예외처리 담당까지 하면 많은 코드가 중복 발생이 된다.
	...
	public String except(Exception ex, Model model) {
		...
		model.addAttribute("exception", ex);				   // 예외처리 메시지를 model에 담기
		log.error(model);									   // 콘솔에 예외처리 메시지 출력
		
		return "error_page"; // error_page.jsp 프론트에 model에 예외 메시지를 보냄
	}
