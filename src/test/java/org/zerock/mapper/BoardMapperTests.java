...
public class BoardMapperTests { // 테스트용 코드
	...
	public void testInsert() {
		BoardVO boardVO = new BoardVO(); // 객체생성
		boardVO.setTitle("매퍼로 만든 제목");
		boardVO.setContent("매퍼로 만든 내용");
		boardVO.setWriter("매퍼로 만든 사용자"); // 객체에 내용 삽입완료
	...
	public void testInsertSelectKey() {
		...
		boardVO.setWriter("번호생성 먼저 사용자"); // 객체에 내용 삽입완료
		
		mapper.insertSelectKey(boardVO); // 번호 먼저 생성 후 insert
		
		log.info("번호생성 먼저 입력된 객체 : " + boardVO);
	}
	...
	public void testDelete() {
		...
		log.info("삭제한 개수 : " + mapper.delete(3));
	}
